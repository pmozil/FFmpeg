...
extern const char *vulkan_dirac_cpy_to_image_8bit;
extern const char *vulkan_dirac_cpy_to_image_10bit;
extern const char *vulkan_dirac_cpy_to_image_12bit;
...
static int init_cpy_shd(DiracVulkanDecodeContext *s, FFVkSPIRVCompiler *spv,
    ...
    if (idx == 0) {
        shd_txt = vulkan_dirac_cpy_to_image_8bit;
    } else if (idx == 1) {
        shd_txt = vulkan_dirac_cpy_to_image_10bit;
    } else if (idx == 2) {
        shd_txt = vulkan_dirac_cpy_to_image_12bit;
    } else {
        return AVERROR_INVALIDDATA;
    }

    RET(compile_shader(s, spv, &s->cpy_to_image[idx],
                        desc, 2, ext, 3,
...
//
//     err = ff_vk_set_descriptor_buffer(
//         &dec->vkctx, &dec->quant_pl, exec, 0, 2, 0, dec->quant_val_buf, 0,
//         dec->quant_val_buf->size, VK_FORMAT_UNDEFINED);
//     if (err < 0)
//         return err;
//
//     err = ff_vk_set_descriptor_buffer(
//         &dec->vkctx, &dec->quant_pl, exec, 0, 3, 0, dec->quant_buf, 0,
//         dec->quant_buf->size, VK_FORMAT_UNDEFINED);
//     if (err < 0)
//         return err;
//
//     err = ff_vk_set_descriptor_buffer(
//         &dec->vkctx, &dec->quant_pl, exec, 0, 4, 0, dec->slice_buf, 0,
//         dec->slice_buf->size, VK_FORMAT_UNDEFINED);
//     if (err < 0)
//         return err;
//
//     err = ff_vk_set_descriptor_buffer(
//         &dec->vkctx, &dec->quant_pl, exec, 0, 5, 0, &dec->subband_info, 0,
...
//     ff_vk_qf_init(&dst_ctx->vkctx, &dst_ctx->qf, VK_QUEUE_COMPUTE_BIT);
//
