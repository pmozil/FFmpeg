...
typedef  struct DiracVulkanDecodeContext {
    ...
    FFVulkanPipeline vert_interleave_pl[9];
    FFVkSPIRVShader vert_interleave_shd[9];

    FFVulkanPipeline horiz_wavelet_pl[9];
    FFVkSPIRVShader horiz_wavelet_shd[9];

    FFVulkanPipeline horiz_interleave_pl[9];
    FFVkSPIRVShader horiz_interleave_shd[9];
...
static const char fidelity_low_x[] = {
    ...
    C(1,    const int v_3 = inBuf[getIdx(plane, x_3, y)];                   )
    C(1,                                                                    )
    C(1,    const int x_2 = clamp(x - 2, 0, dw);                            )
    C(1,    const int v_2 = inBuf[getIdx(plane, x_2, y)];                   )
    C(1,                                                                    )
    C(1,    const int x_1 = clamp(x - 1, 0, dw);                            )
    C(1,    const int v_1 = inBuf[getIdx(plane, x_1, y)];                   )
    C(1,                                                                    )
    ...
    C(1,    const int v1 = inBuf[getIdx(plane, x1, y)];                     )
    C(1,                                                                    )
    C(1,    const int x2 = clamp(x + 2, 0, dw);                             )
    C(1,    const int v2 = inBuf[getIdx(plane, x2, y)];                     )
    C(1,                                                                    )
    C(1,    const int x3 = clamp(x + 3, 0, dw);                             )
    C(1,    const int v3 = inBuf[getIdx(plane, x3, y)];                     )
    C(1,                                                                    )
    C(1,    const int x4 = clamp(x + 4, 0, dw);                             )
    C(1,    const int v4 = inBuf[getIdx(plane, x4, y)];                     )
    C(1,                                                                    )
