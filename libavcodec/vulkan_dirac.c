...
typedef  struct DiracVulkanDecodeContext {
    ...
    AVBufferPool *tmp_buf_pool;
    FFVkBuffer *tmp_buf;
    AVBufferRef *tmp_avbuf;
    FFVkBuffer *tmp_interleave_buf;
    AVBufferRef *tmp_interleave_avbuf;
...
static void free_common(AVCodecContext *avctx)
{
    ...
    av_buffer_unref(&dec->tmp_avbuf);
    av_buffer_unref(&dec->tmp_interleave_avbuf);
    av_buffer_pool_uninit(&dec->tmp_buf_pool);
...
static inline int alloc_tmp_bufs(DiracContext *ctx, DiracVulkanDecodeContext *dec) {
    ...
    err = ff_vk_get_pooled_buffer(&dec->vkctx, &dec->tmp_buf_pool,
                                    &dec->tmp_avbuf,
    ...
    if (err < 0)
        return err;
    dec->tmp_buf = (FFVkBuffer *)dec->tmp_avbuf->data;

    err = ff_vk_get_pooled_buffer(&dec->vkctx, &dec->tmp_buf_pool,
                                    &dec->tmp_interleave_avbuf,
                                    VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
                                        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                                    NULL,
                                    plane_size,
    ...
    if (err < 0)
        return err;
    dec->tmp_interleave_buf = (FFVkBuffer *)dec->tmp_interleave_avbuf->data;
...
static const char legall_vert[] = {
    ...
    C(1,                                                                                )
    C(1,    const int y0 = y;                                                           )
    C(1,    const int in0 = plane_offs[plane] + y0 * plane_strides[plane] + x;          )
    C(1,    const int32_t val0 = inBuf[in0];                                            )
    C(1,                                                                                )
    C(1,    const int y1 = int((y - 1) >= 0) * (y - 1) + int((y - 1) < 0) * 1;          )
    C(1,    const int in1 = plane_offs[plane] + y1 * plane_strides[plane] + x;          )
    C(1,    const int32_t val1 = inBuf[in1];                                            )
    C(1,                                                                                )
    C(1,    const int y2 = y + 1;                                                       )
    C(1,    const int in2 = plane_offs[plane] + y2 * plane_strides[plane] + x;          )
    C(1,    const int32_t val2 = inBuf[in2];                                            )
    C(1,                                                                                )
    C(1,    const int y3 = int((y + 2) < h) * (y + 2) + int((y + 2) >= h) * (h - 2);    )
    C(1,    const int in3 = plane_offs[plane] + y3 * plane_strides[plane] + x;          )
    C(1,    const int32_t val3 = inBuf[in3];                                            )
    C(1,                                                                                )
    C(1,    const int32_t out0 = val0 - ((val1 + val2 + 2) >> 2);                       )
    C(1,    outBuf[in0] = out0;                                                         )
    C(1,                                                                                )
    C(1,    const int32_t out1 = val2 + ((val0 + val3 + 1) >> 1);                       )
    C(1,    outBuf[in2] = out0;                                                         )
    C(0, }                                                                              )
};
...
static const char legall_horiz[] = {
    C(0, void idwt_vert(int plane, int x, int y) {                                      )
    C(1,    const int w = plane_sizes[plane].w;                                         )
    C(1,                                                                                )
    C(1,    const int x0 = y;                                                           )
    C(1,    const int in0 = plane_offs[plane] + y * plane_strides[plane] + x0;          )
    C(1,    const int32_t val0 = inBuf[in0];                                            )
    C(1,                                                                                )
    C(1,    const int x1 = int((x - 1) >= 0) * (x - 1) + int((x - 1) < 0) * 1;          )
    C(1,    const int in1 = plane_offs[plane] + y * plane_strides[plane] + x1;          )
    C(1,    const int32_t val1 = inBuf[in1];                                            )
    C(1,                                                                                )
    C(1,    const int x2 = x + 1;                                                       )
    C(1,    const int in2 = plane_offs[plane] + y * plane_strides[plane] + x2;          )
    C(1,    const int32_t val2 = inBuf[in2];                                            )
    C(1,                                                                                )
    C(1,    const int x3 = int((x + 2) < w) * (x + 2) + int((x + 2) >= w) * (w - 2);    )
    C(1,    const int in3 = plane_offs[plane] + y3 * plane_strides[plane] + x;          )
    C(1,    const int32_t val3 = inBuf[in3];                                            )
    C(1,                                                                                )
    C(1,    const int32_t out0 = val0 - ((val1 + val2 + 2) >> 2);                       )
...
static const char haari_horiz[] = {
    ...
    C(1,    ivec2 p_size = plane_sizes[plane];                                  )
...
static const char haari_vert[] = {
    C(0, void idwt_vert(int plane, int x, int y) {                              )
    C(1,    int offs0 = plane_offs[plane] + plane_strides[plane] * y + x;       )
    C(1,    int offs1 = offs0 + plane_strides[plane];                           )
...
static av_always_inline int inline wavelet_haari_pass(DiracVulkanDecodeContext *dec,
    ...
    for (int i = ctx->wavelet_depth - 1; i >= 0; i--) {
        ...
        dec->pConst.dw[0] = dec->pConst.real_plane_dims[0] >> 1;
        dec->pConst.dw[1] = dec->pConst.real_plane_dims[2] >> 1;
        dec->pConst.dw[2] = dec->pConst.real_plane_dims[4] >> 1;
        ...
        vk->CmdDispatch(exec->buf,
                        ctx->plane[0].idwt.width  >> (i + 2),
                        ctx->plane[0].idwt.height >> (i + 3),
                        1);
        ...
        /**/
        /*vk->CmdDispatch(exec->buf,*/
        /*                ctx->plane[0].idwt.width  >> (i + 3),*/
        /*                ctx->plane[0].idwt.height >> (i + 2),*/
        /*                1);*/
    }
...
static int vulkan_dirac_end_frame(AVCodecContext *avctx) {
    ...
    err =ff_vk_exec_add_dep_buf(&dec->vkctx, exec, &dec->tmp_avbuf, 1, 1);
    if (err < 0) {
        av_log(&dec->vkctx, AV_LOG_ERROR, "AAAAAA\n");
        goto fail;
      }

    err =ff_vk_exec_add_dep_buf(&dec->vkctx, exec, &dec->tmp_interleave_avbuf, 1, 1);
    if (err < 0) {
        av_log(&dec->vkctx, AV_LOG_ERROR, "BBBBBB\n");
        goto fail;
      }
